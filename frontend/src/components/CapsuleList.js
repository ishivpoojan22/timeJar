...
export default function CapsuleList() {
  ...
  const handleRating = (id, rating) => {
    ...
    setActivityLog((prev) => [
      `⭐ Rated capsule ${id} with ${rating} stars`,
      ...prev,
    ]);
  };
  ...
  return (
    <div className="capsule-list-container">
      <div className="capsule-summary">
        ...
        <p>
          Total: {capsules.length} | 📬 Opened: {openedCount} | 🔒 Pending:{" "}
          {pendingCount}
        </p>
        <div className="summary-bars">
          <div
            className="bar opened"
            style={{ width: `${(openedCount / capsules.length) * 100 || 0}%` }}
          />
          <div
            className="bar pending"
            style={{ width: `${(pendingCount / capsules.length) * 100 || 0}%` }}
          />
        </div>
      </div>
            ...
            filteredCapsules.map((cap) => {
              ...
              const handleLocationCheckIn = async () => {
                ...
                try {
                  ...
                  if (!geoData.length) {
                    alert(
                      `Could not find coordinates for '${cap.triggerValue}'. Please check the location name.`
                    );
                    return;
                  }
                  loc = {
                  ...
                  alert(
                    "Failed to look up location coordinates. Please try again later."
                  );
                  return;
                }
                if (!navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(
                  async (position) => {
                    ...
                    if (
                      geoType === "country" ||
                      (geoClass === "boundary" &&
                        geoType === "administrative" &&
                        geoDisplayName
                          .toLowerCase()
                          .includes(cap.triggerValue.toLowerCase()))
                    ) {
                      radius = 1000;
                    }
                    ...
                    if (distance > radius) {
                      alert(
                        `You are not in the expected location (${cap.triggerValue}). Please travel to ${cap.triggerValue} to open this capsule.`
                      );
                      setActivityLog((prev) => [
                        `❌ Check-in failed: Not in ${
                          cap.triggerValue
                        } (distance: ${distance.toFixed(
                          1
                        )} km, allowed: ${radius} km)`,
                        ...prev,
                      ]);
                      return;
                    }
                    const userId = cap.userId;
                    const res = await API.post("/capsule/checkin", {
                      userId,
                      location: cap.triggerValue,
                      latitude,
                      longitude,
                    });
              ...
              return (
                <div
                  ...
                  className={`capsule-card ${
                    cap.isDelivered ? "delivered" : "locked"
                  }`}
                >
                  <h3>{cap.title}</h3>
                  <p className="status">
                  ...
                  <p>
                    <strong>Trigger:</strong> {cap.triggerType} –{" "}
                    {cap.triggerValue}
                  </p>
                  ...
                  {!cap.isDelivered && cap.triggerType === "date" && (
                    <>
                      ...
                      <p className="countdown-text">
                        ⏳ Unlocks in: <strong>{timeLeft}</strong>
                      </p>
                    </>
                  )}
                  ...
                  <div className="capsule-actions">
                    <button
                      onClick={() => handleViewMessage(cap.id)}
                      className="view-button"
                    >
                      {expandedId === cap.id ? <FaEyeSlash /> : <FaEye />}{" "}
                      {expandedId === cap.id ? "Hide" : "View"}
                    </button>
                    <button
                  ...
                  <div className="rating-container">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <FaStar
                        key={star}
                        className="star-icon"
                        onClick={() => handleRating(cap.id, star)}
                      />
                    ))}
                  </div>
                </div>
